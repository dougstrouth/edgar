...
def main():
    """Run the Polygon/Massive.com capacity test."""
    print("=" * 80)
    ...
    # Calculate capacity
    if successes > 0 and len(tickers_to_test) > 0:
        ...
        try:
            with ManagedDatabaseConnection(db_path_override=config.DB_FILE_STR, read_only=True) as conn:
                if conn:
                    result = conn.execute("SELECT COUNT(DISTINCT ticker) FROM tickers").fetchone()
                    if result:
                        total_tickers = result[0]
